...
# Exposure direct
model_direct_count <- firearm_pointed ~
  exposure_i +
...
# Exposure indirect
model_indirect_count <- firearm_pointed ~
  exposure_d +
  # I(exposure_d * officer_nyears_sd) +
  # nevents +
  officer_nyears +
  officer_nyears2 + 
  officer_po +
...
set.seed(77173)
ans_direct_count <- clogit_perm(nperms, model_direct_count, dat = model_data, ncpus = ncpus)

set.seed(77173)
ans_indirect_count <- clogit_perm(nperms, model_indirect_count, dat = model_data, ncpus = ncpus)
